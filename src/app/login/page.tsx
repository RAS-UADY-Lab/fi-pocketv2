...
export default function LoginPage() {
  ...
  const [step, setStep] = useState<1 | 2>(1); // Step 1: Credenciales, Step 2: Selección de Campus / Código
  ...
  // Estados de datos dinámicos (SaaS)
  ...
  // --- 2. REGISTRO - PASO 1: Validar Dominio ---
  const avanzarRegistro = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Verificamos si el dominio le pertenece a alguna Universidad (Organización) cliente
      ...
      if (orgEncontrada) {
        // Tiene correo institucional oficial, buscamos sus campus
        ...
        // No tiene correo oficial, requerirá código de acceso
        setOrgMatched(null);
      }

      setStep(2); // Avanzamos a la siguiente pantalla
    } catch (err: any) {
  ...
  // --- 3. REGISTRO - PASO 2: Finalizar Creación ---
  const completarRegistro = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (orgMatched) {
        // Pertenece a una universidad, tomamos el campus que seleccionó en el dropdown
        finalTenantId = tenantSeleccionado;
      } else {
        // No tiene dominio oficial, validamos el código de invitación contra la base de datos
      ...
      // Creamos el usuario inyectando el tenant_id y el ROL inicial en su metadata
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          data: { 
            ...
            rol: 'estudiante' // <-- INYECCIÓN DE ROL SEGURO
          } 
        }
      });
      if (error) throw error;

      // Por seguridad, forzamos la actualización directa de la tabla perfiles
      if (data.user) {
        await supabase.from("perfiles").update({ 
          tenant_id: finalTenantId,
          rol: 'estudiante' // <-- RESPALDO DE INYECCIÓN DE ROL
        }).eq("id", data.user.id);
      }
  ...
  return (
    <main className="min-h-[calc(100dvh-4.25rem)] md:min-h-[100dvh] flex flex-col justify-center items-center p-4 md:p-8 bg-slate-50 relative overflow-hidden">
      ...
      {/* CORRECCIÓN: Fondo Premium Nodum dinámico */}
      ...
      <div className="bg-white border border-slate-200 rounded-[2.5rem] w-full max-w-md p-6 md:p-10 shadow-2xl relative z-10">
        ...
        <form onSubmit={isLogin ? handleLogin : step === 1 ? avanzarRegistro : completarRegistro} className="space-y-5">
          ...
          {/* VISTA 1: Login o Paso 1 de Registro */}
          {(isLogin || step === 1) && (
          ...
          {/* VISTA 2: Paso 2 de Registro (Campus o Código) */}
          {!isLogin && step === 2 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-5">
              
              {orgMatched ? (
                // ESCENARIO A: Universidad Reconocida
                ...
                // ESCENARIO B: Necesita Código de Institución
          ...
          {/* CORRECCIÓN: Botón principal con degradado y opacidad dinámica */}
          <button
            type="submit"
            disabled={loading}
